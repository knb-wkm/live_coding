...
void main(void)
{
    ...
    float u      = (uv.x-0.001)/5;
    float wave   = -1.5*abs(texture2D(iChannel0,vec2(u,1.75)).x);
    float inside = smoothbump(0.5, 1.0,uv.x);
    wave         = wave*inside;
    float line   = smoothbump(0.0,(9.0/iResolution.y), wave + uv.y - deltay);
    vec3  lcolor = line*vec3(0.9,0.9,0.9);
    float mask   = smoothstep(0.0, (10.0/iResolution.y)/2.0, wave + uv.y - deltay);
